...
namespace Cosmosdb.development.Models
{
    public static class DocumentsDemo
    {
        ...
        private async static Task CreateDocuments()
        {
            ...
            var container = Shared.Client.GetContainer("DemoCosmosDB", "stores");
            ...
            await container.CreateItemAsync(documentDynamic, new PartitionKey("11229"));
            Console.WriteLine($"Created new document {documentDynamic.id} from dynamic object");
            ...
            var documentObject = JsonConvert.DeserializeObject<JObject>(document2Json);
            await container.CreateItemAsync(documentObject , new PartitionKey("11229") );
            Console.WriteLine($" Created new document {documentObject["id"].Value<string>()} from JSON string");
        ...
        private static async Task QueryDocuments()
        {
            ...
            Console.WriteLine("Querying for new customer documents");
            var sql = "SELECT * FROM c WHERE STARTSWITH(c.name,'New Customer') = true";
