...
namespace Cosmosdb.development.Models
{
	public static class ContainersDemo
	{
		public async static Task Run()
		{
			...
			await CreateContainer("stores");
			await CreateContainer("stores2", 1000, "/state");
			await ViewContainers();

			//await DeleteContainer("stores");
			//await DeleteContainer("stores2");
			await ViewContainers();
		}
		...
		{
			Console.WriteLine();
			Console.WriteLine(">>> View Containers>>>");

			var database = Shared.Client.GetDatabase("DemoCosmosDB");
			var iterator = database.GetContainerQueryIterator<ContainerProperties>();
		...
		private async static Task ViewContainer(ContainerProperties containerProperties)
		{
			...
			Console.WriteLine($" Parition Key: {containerProperties.PartitionKeyPath}");

			var container = Shared.Client.GetContainer("DemoCosmosDB", containerProperties.Id);
		...
		{
			Console.WriteLine();
			Console.WriteLine($" >>> Create Container {containerId} in DemoCosmosDB <<<<");
			Console.WriteLine();
            Console.WriteLine($" Throughput: {throughput} RU/sec");
            Console.WriteLine($" Partition key: {partitionKey}");
